// compile command
//cl.exe main.cpp /EHsc user32.lib kernel32.lib gdi32.lib opengl32.lib glu32.lib
//
...
TCHAR windowTitle[] = TEXT("OpenGL First Polygon");
...
GLvoid drawTriangle(GLvoid)
{
    ...
    glTranslatef(-1.5f, 0.0f, -6.0f); // Move left by 1.5 units and into the screen by 6.0 units.
...
{
    glLoadIdentity(); // Reset current model-view matrix.
    glTranslatef(1.5f, 0.0f, -6.0f); // Move right by 1.5 units and into the screen by 6.0 units.
    glBegin(GL_QUADS);
...
BOOL createGLWindow(LPCSTR title, int width, int height, int bits, bool fullscreenFlag)
{
    ...
    resizeGLScene(width, height); // Setup perspective OpenGL screen.
